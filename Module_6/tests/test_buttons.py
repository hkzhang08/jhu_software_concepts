...
def _recorded_publish(tasks):
    """Return a fake publisher that records queued button tasks."""

    def _publish(task_name, payload=None):
...
def test_post_pull_data_triggers_loader_with_scraped_rows(monkeypatch, tmp_path):
    ...
    app = website.create_app(
        run_pull_pipeline_fn=website.run_pull_pipeline,
...
def test_post_pull_data_with_explicit_json_accept():
    """Explicit JSON Accept header follows the JSON response branch."""
    website.PULL_STATE["status"] = "idle"
    app = website.create_app(
        run_pull_pipeline_fn=lambda: None,
        fetch_metrics_fn=_fake_metrics,
    )
    app.config["TESTING"] = True
    client = app.test_client()
...
def test_pull_data_redirects_for_html_form_posts():
    """Browser-style HTML form submits are redirected back to index."""
    tasks = []
    website.PULL_STATE["status"] = "idle"
    app = website.create_app(
        fetch_metrics_fn=_fake_metrics,
        publish_task_fn=_recorded_publish(tasks),
    )
    app.config["TESTING"] = True
    client = app.test_client()
