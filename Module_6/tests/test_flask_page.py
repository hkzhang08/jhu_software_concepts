...
def test_pull_data_route_returns_ok_json():
    """POST /pull-data queues a background request when idle."""
    tasks = []

    def fake_publish(task_name, payload=None):
        tasks.append((task_name, payload or {}))
        return {"task_name": task_name, **(payload or {})}

    website.PULL_STATE["status"] = "idle"
    app = website.create_app(publish_task_fn=fake_publish)
    app.config["TESTING"] = True
    client = app.test_client()
    ...
    assert resp.get_json() == {
        "status": "queued",
        "task": website.SCRAPE_TASK_NAME,
    }
    assert tasks == [(website.SCRAPE_TASK_NAME, {})]


def test_pull_data_route_returns_503_when_queue_publish_fails():
    """POST /pull-data returns 503 when the queue publish fails."""
    def boom(*_args, **_kwargs):
        raise website.PublishError("broker unavailable")

    website.PULL_STATE["status"] = "idle"
    app = website.create_app(publish_task_fn=boom)
    app.config["TESTING"] = True
    client = app.test_client()

    resp = client.post("/pull-data")
    assert resp.status_code == 503
    assert resp.get_json() == {"error": website.PUBLISH_FAILED_ERROR}
...
def test_update_analysis_route_redirects_to_index():
    ...
    assert tasks == [(website.RECOMPUTE_TASK_NAME, {})]


def test_update_analysis_route_returns_queued_json_for_explicit_json():
    """POST /update-analysis queues a background refresh for AJAX callers."""
    tasks = []
    ...
    assert resp.get_json() == {
        "status": "queued",
        "task": website.RECOMPUTE_TASK_NAME,
    }
    assert tasks == [(website.RECOMPUTE_TASK_NAME, {})]


def test_update_analysis_route_returns_503_when_queue_publish_fails():
    """POST /update-analysis returns 503 when the queue publish fails."""
    def boom(*_args, **_kwargs):
        raise website.PublishError("broker unavailable")
